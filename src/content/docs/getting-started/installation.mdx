...
:::tip[Solo + Cloud]
Add `providers` and `vertex_ai` to also route cloud models (Gemini, Claude) through candela-local. See [candela-local docs](/candela-docs/components/candela-local/).
:::
...
See [candela-server docs](/candela-docs/components/candela-server/) for Cloud Run deployment with Terraform.
...
:::note[You're ready!]
Head to the [Quick Start](/candela-docs/getting-started/quickstart/) to send your first traced LLM request.
:::
