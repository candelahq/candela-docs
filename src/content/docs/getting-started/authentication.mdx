...
When your team runs a Candela server behind [Identity-Aware Proxy (IAP)](https://cloud.google.com/iap), `candela` uses a **dual-token** authentication flow. IAP requires its own OIDC token, but it also _replaces_ the `Authorization` header with its own JWT — so `candela` must send the developer's real identity through a separate header.
...
The service account must have a custom **`iapIdTokenCreator`** IAM role that grants only `iam.serviceAccounts.getOpenIdToken` — **not** `getAccessToken`. This ensures developers can authenticate through IAP but cannot impersonate the SA for direct LLM API access, which would bypass proxy budget enforcement. See [Security — SA Impersonation](/architecture/security/#strategy-15-dual-token-for-iap-user-adc--iap_service_account) for details.
</Aside>

<Aside type="note">
**Why `X-Candela-Auth`?** IAP strips the original `Authorization` header and replaces it with its own identity JWT. Without a separate header, the server would only see IAP's service identity — not the developer who made the request. `X-Candela-Auth` carries the developer's real OAuth2 access token through IAP untouched.
</Aside>
