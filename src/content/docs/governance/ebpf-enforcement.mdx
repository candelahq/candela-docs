...
Candela will add **kernel-level enforcement** to guarantee that all LLM API traffic flows through the proxy — making observability, budget controls, and policy enforcement impossible to bypass, even by misconfigured or malicious workloads.
...
Applications are configured to point at the sidecar directly (e.g., `OPENAI_BASE_URL=http://localhost:8181/proxy/openai/v1`). No iptables redirect, no TLS interception. This is how Candela works today.
...
|-------|-----------|--------|
| 0 | Config schema design (`candela-policy.yaml`) | <Badge text="In Design" variant="caution" /> |
| 1 | Extend `Provider` struct with host/intercept fields | <Badge text="Planned" variant="default" /> |
| 2 | Config file loading in `candela-sidecar` | <Badge text="Planned" variant="default" /> |
| 3 | Helm chart with enforcement templates | <Badge text="Planned" variant="default" /> |
| 4 | Transparent listener (Go — SNI routing) | <Badge text="Planned" variant="default" /> |
| 5 | Transparent listener (Rust — `rustls`) | <Badge text="Planned" variant="default" /> |
