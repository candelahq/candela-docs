...
budget:
  timezone: "America/New_York"  # example (defaults to UTC if omitted)
```

:::note[Budget timezone]
The `budget.timezone` config controls when daily budget periods reset. Use any valid IANA timezone identifier (e.g., `America/New_York`, `Europe/London`, `Asia/Tokyo`). Defaults to UTC if omitted.

This affects:
- When daily spend counters reset to $0.00
- The `period_key` value returned by `GetMyBudget` (e.g., `2026-07-04`)
- The `period_resets_at` timestamp in budget responses

The server uses the host OS timezone database (`/usr/share/zoneinfo`). If deploying in minimal containers, ensure timezone data is available or use UTC.
:::
