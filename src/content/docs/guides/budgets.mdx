...
Candela provides a complete cost governance system — per-user daily budgets, one-time grants, threshold alerts, rate limiting, and an immutable audit trail. All budget data lives in **Firestore** and is enforced at the proxy layer in real time.
...
4. All updates are **transactional** (Firestore transaction)
...
Rate limits are stored as minute-window counters in Firestore with a 2-minute TTL.
...
users/{email}/budgets/{period_key}     → BudgetRecord  (e.g. "2026-05-04")
users/{email}/grants/{grant_id}        → GrantRecord
users/{email}/audit/{audit_id}         → AuditRecord
global_audit/{audit_id}                → AuditRecord   (survives user deletion)
rate_limit/{email}:{window_key}        → Counter        (TTL: 2 min)
```
