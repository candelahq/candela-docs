...
For Claude Sonnet 4 ($3/MTok input):
...
For Google Gemini models, cached content is priced at a discount from the standard input rate. Configure the discount factor:

```bash
# Set Gemini cache discount (0.0 = free, 1.0 = no discount)
curl -X POST http://localhost:8181/_local/api/config \
  -H "Content-Type: application/json" \
...
<Aside type="note">
The `gemini_cache_discount` value must be between `0.0` and `1.0`. Invalid values return a `400 Bad Request` response. The default is `0.25` (75% discount on cached tokens), matching Google's current Gemini pricing.
</Aside>
